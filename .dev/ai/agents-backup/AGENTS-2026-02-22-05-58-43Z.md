...
- **NEVER use Haiku or Sonnet** for subtasks (always use Opus)
...
**Trigger phrases:** "sync rules", "sync agents", "sync agents.md", "update rules", "update agents.md", "update the agents.md", "sync from global", "merge global rules", "update project agents", "update agents.md from global"
**External File:** `~/.agents/modes/AGENTS-SYNC-MODE.md`

**Purpose:** Sync project `AGENTS.md` with the immutable global copy (file copy, no merge).
...
**Key Rules:**
- ❌ NO branch creation without explicit permission
- ✅ Commits allowed only when explicitly instructed
- ❌ NO automatic commits
- 🚨 **ZERO AI ATTRIBUTION POLICY** - User's work only
- ✅ Clean messages, atomic changes, run tests first

**When to read full guide:**
- Git workflows and permissions
- ZERO AI ATTRIBUTION requirements
- Branch/commit authorization issues
