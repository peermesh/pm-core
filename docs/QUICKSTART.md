...
8. **Enable modules**
   ```bash
   ./launch_pm-core.sh module enable hello-core
   ```
...
```bash
./launch_pm-core.sh up --profile=postgresql
```
...
# MySQL
./launch_pm-core.sh up --profile=mysql
```
...
./launch_pm-core.sh up --profile=mysql --example=ghost
```
