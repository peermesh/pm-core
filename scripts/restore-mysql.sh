#!/bin/bash
# ==============================================================
# MySQL Restore Script (Top-Level Operational)
# ==============================================================
# Purpose: Restore MySQL databases from a gzip-compressed
#          SQL dump file with integrity verification and
#          interactive confirmation.
#
# Usage:   ./scripts/restore-mysql.sh <backup_file.dump.gz>
#          ./scripts/restore-mysql.sh --no-confirm <backup_file>
#
# Referenced by:
#   - docs/DEPLOYMENT.md
...
BACKUP_DIR="${BACKUP_DIR:-/var/backups/pmdl/daily/mysql}"
...
# Read MySQL root password from secrets
read_mysql_password() {
    local password=""

    if [[ -f "/opt/peermesh/secrets/mysql_root_password" ]]; then
        password="$(cat /opt/peermesh/secrets/mysql_root_password)"
    elif [[ -f "${PROJECT_ROOT}/secrets/mysql_root_password" ]]; then
        password="$(cat "${PROJECT_ROOT}/secrets/mysql_root_password")"
    fi

    if [[ -z "$password" ]]; then
        error_exit "MySQL root password not found. Checked:
  /opt/peermesh/secrets/mysql_root_password
...
Restore a MySQL database from a gzip-compressed SQL dump.
...
Environment Variables:
    CONTAINER_NAME    MySQL container (default: pmdl_mysql)
    BACKUP_DIR        Backup search directory

Examples:
    $0                                                # List recent backups
    $0 /var/backups/pmdl/daily/mysql/mysql-2026.dump.gz
    $0 --no-confirm /var/backups/pmdl/daily/mysql/mysql-2026.dump.gz
...
main() {
    ...
    if [[ "$no_confirm" != true ]]; then
        echo ""
        echo "WARNING: This will restore the MySQL database from the backup above."
    ...
    gunzip -c "$backup_file" | docker exec -i "$CONTAINER_NAME" mysql -u root -p"$MYSQL_PASSWORD" \
    ...
    echo "  docker exec $CONTAINER_NAME mysql -u root -p\"\$(cat /opt/peermesh/secrets/mysql_root_password)\" -e 'SHOW DATABASES;'"
    echo "  docker exec $CONTAINER_NAME mysql -u root -p\"\$(cat /opt/peermesh/secrets/mysql_root_password)\" -e 'SELECT @@version;'"
    echo ""
}
