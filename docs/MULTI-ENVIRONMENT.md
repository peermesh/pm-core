...
Use the `env` command in `launch_peermesh.sh`:

```bash
# Switch to local development
./launch_peermesh.sh env local

# Switch to staging
./launch_peermesh.sh env staging

# Switch to production
./launch_peermesh.sh env production

# List available environments
./launch_peermesh.sh env
```
...
When you enable a module via `./launch_peermesh.sh module enable <name>`, the foundation's `DOMAIN` is automatically propagated to the module's `.env` file. This means:
