...
## 🚫 CRITICAL OVERRIDE: NO INTERNAL ARTIFACT DIRECTORIES (Claude, Gemini, GPT, Cursor, Codex, Copilot — ALL MODELS)

**THIS IS A CRITICAL OWNER DIRECTIVE. NO EXCEPTIONS.**

**NO agent may EVER write planning documents, brainstorms, task lists, research notes, findings, reports, or any markdown output to its own internal artifact or "brain" directory.** Every AI platform maintains a hidden, model-specific storage location (e.g., `~/.gemini/antigravity/brain/`, Claude's artifact panel, Cursor's composer artifacts, etc.). **These locations are invisible to the user and to other agents. Using them defeats the entire purpose of GAS.**

**THE CORRECT BEHAVIOR IS:**

All generated documents MUST be written as **standard files** to the **current project's GAS directory structure** using absolute filesystem paths:

- **Findings / brainstorms / research:** `.dev/ai/findings/`
- **Work orders:** `.dev/ai/workorders/`
- **Proposals:** `.dev/ai/proposals/`
- **Handoffs:** `.dev/ai/handoffs/`
- **Changelogs:** `.dev/ai/changelogs/`
- **General documentation:** `docs/`

**WHY THIS RULE EXISTS:**
1. **Visibility.** Documents written to internal artifact stores are inaccessible to the user outside that specific tool session.
2. **Cross-agent collaboration.** Other agents cannot read artifacts locked inside a different model's internal storage.
3. **Persistence.** GAS directory files live in the repo, survive across sessions, and are version-controlled.

**If your platform offers an "artifact" or "brain" feature, IGNORE IT for document creation. Write real files to real paths.**
