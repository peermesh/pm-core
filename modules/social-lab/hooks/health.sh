#!/bin/bash
# ==============================================================
# Social Lab Module - Health Check Hook
# ==============================================================
...
#
# Checks:
#   1. Container social-lab-app is running
...
check_container_running() {
    if docker ps --filter "name=${CONTAINER_NAME}" --filter "status=running" \
        ...
        # Get uptime (container start time)
...
check_http_endpoint() {
    ...
    # Check HTTP /health response from inside the container
    local response
    response=$(docker exec "${CONTAINER_NAME}" wget --quiet --tries=1 --timeout=5 -O - "http://127.0.0.1:3000/health" 2>/dev/null || printf "")

    if [[ -n "$response" ]]; then
        HTTP_STATUS="ok"

        # Try to extract database status from the health response
        # The app returns JSON with checks.database.status
