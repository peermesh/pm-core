-- ============================================================
-- Social Lab Module: Initial Schema Migration
...
--   PRIVATE (Social Lab internal only):
...
COMMENT ON SCHEMA social_federation IS
  'PRIVATE surface: federation protocol state (ActivityPub actors, AT Protocol sync). Social Lab internal only.';

CREATE SCHEMA IF NOT EXISTS social_keys;
COMMENT ON SCHEMA social_keys IS
  'PRIVATE surface: cryptographic key storage metadata. Actual key material lives in the app key store, not in SQL. Social Lab internal only.';

CREATE SCHEMA IF NOT EXISTS social_pipeline;
COMMENT ON SCHEMA social_pipeline IS
  'PRIVATE surface: Omni-Account pipeline execution, Pod sync state, schema migration tracking. Social Lab internal only.';
