...
//   - Profile synchronization (display name, avatar) from Social -> Matrix
...
//      and room directory queries. Social acts as an Application Service.
...
//   2. Matrix homeserver — External service, NOT inside Social. Runs as a
//      separate Docker Compose service in Core.
...
//   3. Application Service (AS) registration — Social registers as a Matrix
...
//
// Deployment modes:
//   Mode 1 (VPS): Full — Self-hosted homeserver in Core, full provisioning + sync.
...
//   - NO messaging, chat, or real-time communication in Social.
//   - NO Matrix E2EE (Olm/Megolm) key management in Social.
//   - NO room creation, management, or participation logic.
//   - Homeserver is an EXTERNAL service in Core, not inside Social.
//   - Profile sync is ONE-DIRECTIONAL: Social -> Matrix only.
...
export class MatrixAdapter extends StubProtocolAdapter {
  constructor() {
    super({
      ...
      stubNote: 'Matrix homeserver is not configured. Matrix ID provisioning, profile sync, and identity bridging require a running Matrix homeserver with admin API access. Social registers as an Application Service. See F-015 blueprint. Note: This adapter is IDENTITY BRIDGE ONLY — no chat/messaging functionality.',
    });
  }
  ...
  async provisionIdentity(profile) {
    return {
      ...
      metadata: {
        ...
        profileSync: {
          direction: 'Social -> Matrix (one-directional)',
          fields: {
        ...
        identityBridge: {
          ...
          matrixToWebid: '.well-known/matrix-webid endpoint on Social domain',
        },
  ...
  async healthCheck() {
    return {
      ...
      details: {
        ...
        deploymentModes: {
          vps: 'Full (self-hosted homeserver in Core, full provisioning + sync)',
