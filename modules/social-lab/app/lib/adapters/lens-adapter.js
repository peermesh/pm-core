...
//      users, Social Lab generates an embedded smart account wallet transparently.
...
//   5. Gas Sponsorship Relayer — Social Lab subsidizes Polygon gas fees for
...
//   - Lens follow -> Social Lab: monitor Lens indexer for follow events targeting
//     local profiles, create corresponding follow in Solid + AP.
//   - Social Lab follow -> Lens: execute Lens follow transaction via dispatcher
...
//   - Deletion: Social Lab can hide a Lens publication but cannot delete from Arweave.
//     Pod copy is the mutable canonical version.
//
// Collect integration:
//   - Social Lab posts cross-posted to Lens can be collected (minted) by Lens users.
    ...
    return {
      ...
      metadata: {
        ...
        walletAbstraction: {
          ...
          gasSponsorship: 'Social Lab relayer pays Polygon gas for profile creation and initial interactions.',
        },
        profileMint: {
          chain: 'Polygon (ERC-721)',
          handle: 'Mapped from Social Lab username: lens/{handle}',
    ...
    return {
      ...
      details: {
        ...
        walletSecurity: {
          ...
          externalWallet: 'Private key never touches Social Lab — only signing requests',
