...
The `launch_docker_lab_core.sh` script provides module management commands:
...
Enable your module with `launch_docker_lab_core.sh module enable my-module` and verify that services start, health checks pass, and Traefik routing works (if applicable). Run the health hook manually to confirm it produces valid output.
...
- [CLI Reference](cli.md) -- `launch_docker_lab_core.sh` module management commands
