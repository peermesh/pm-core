...
//   3. Provider MSA Registration — Social Lab must register as a DSNP Provider
...
//
// Architecture:
//   - DSNP stores ONLY pointers (announcements) on-chain. Social Lab hosts
//     the actual content in Solid Pods. This is a natural fit: Social Lab is
//     the content authority, DSNP is the announcement fabric.
//   - Social Lab operates as a DSNP Provider: users delegate to Social Lab
...
//
// Delegation model:
//   - During Omni-Account creation, user grants delegation to Social Lab Provider.
...
//   - Social Lab NEVER holds user private keys — only provider signing authority.
//
// Announcement types:
...
//   Mode 4 (BYOP): Proxy — Social Lab provisions DSNP identity on behalf of BYOP
//     user (similar to Proxy Actor pattern). Delegation to Social Lab Provider.
//
// Capacity management:
...
export class DsnpAdapter extends StubProtocolAdapter {
  ...
  async provisionIdentity(profile) {
    return {
      ...
      metadata: {
        stub: true,
        note: 'DSNP User ID (MSA — Message Source Account) would be provisioned here via Frequency blockchain. Steps: (1) createMsa() on Frequency, (2) grantDelegation() to Social Lab Provider, (3) publish initial Profile announcement. User ID stored in WebID as pmsl:dsnpUserId. Delegation is revocable.',
        ...
        providerModel: {
          description: 'Social Lab operates as a registered DSNP Provider on Frequency',
        ...
        socialGraph: {
          ...
          crossProtocolSync: 'Follow in Social Lab -> update Pod + AP Follow + DSNP graph change',
        },
    ...
    return {
      ...
      details: {
        ...
        deploymentModes: {
          ...
          byop: 'Proxy (Social Lab provisions DSNP identity on behalf of BYOP user)',
        },
