...
//   - NO messaging, chat, or conversation management in Social Lab.
...
//   - Profile sync is ONE-DIRECTIONAL: Social Lab -> external discovery mechanisms.
  ...
  async provisionIdentity(profile) {
    return {
      ...
      metadata: {
        ...
        walletTypes: {
          external: 'User connects existing wallet (MetaMask, Coinbase Wallet, WalletConnect). Private key NEVER touches Social Lab.',
          managed: 'Social Lab generates secp256k1 keypair CLIENT-SIDE ONLY. Stored in SubtleCrypto / IndexedDB. Server never has access.',
        },
        identityBridge: {
          webidToXmtp: 'Read pmsl:xmtpAddress from WebID card or DATA-002 local index',
          xmtpToWebid: '.well-known/xmtp-profiles endpoint on Social Lab domain',
        ...
        profileSync: {
          direction: 'Social Lab -> external discovery (one-directional)',
          mechanisms: [
            'ENS text records (name, avatar, description, url) — requires on-chain tx, opt-in',
            'WebID as profile URL (authoritative source)',
            'XMTP custom content type for profile (provided by chat module, not Social Lab)',
          ],
        },
  ...
  async healthCheck() {
    return {
      ...
      details: {
        ...
        securityModel: {
          externalWallet: 'Private key NEVER touches Social Lab. Only eth_accounts and personal_sign/eth_signTypedData_v4 requested.',
