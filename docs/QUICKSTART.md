...
8. **Enable modules**
   ```bash
   ./launch_core.sh module enable hello-core
   ```
...
```bash
./launch_core.sh up --profile=postgresql
```
...
# MySQL
./launch_core.sh up --profile=mysql
```
...
./launch_core.sh up --profile=mysql --example=ghost
```
