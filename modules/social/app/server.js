...
try {
  ...
  console.warn('[social] WebPush initialization deferred:', err.message);
}
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`[social] Module: ${MODULE} v${VERSION}`);
  console.log(`[social] Landing page: http://0.0.0.0:${PORT}/`);
  console.log(`[social] Health endpoint: http://0.0.0.0:${PORT}/health`);
  ...
  registerSelfInstance().then(() => {
    console.log('[social] Instance self-registration complete');
  }).catch((err) => {
    console.warn('[social] Instance self-registration deferred:', err.message);
  });
});
...
function shutdown(signal) {
  ...
  server.close(() => {
    console.log('[social] HTTP server closed');
    process.exit(0);
  });
  // Force exit after 10 seconds if graceful shutdown stalls
  setTimeout(() => {
    console.error('[social] Forced shutdown after timeout');
    process.exit(1);
  }, 10000).unref();
}
