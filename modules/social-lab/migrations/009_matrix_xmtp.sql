-- ============================================================
-- Social Lab Module: Matrix + XMTP Identity Bridge
...
-- Social Lab. Matrix and XMTP messaging is handled by external
...
COMMENT ON COLUMN social_profiles.profile_index.matrix_id IS
    'Matrix user ID (@user:domain). Identity bridge only -- no messaging in Social Lab. See F-015.';
...
COMMENT ON COLUMN social_profiles.profile_index.xmtp_address IS
    'XMTP-capable Ethereum address (0x...). Identity bridge only -- no messaging in Social Lab. See F-016.';
